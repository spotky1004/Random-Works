...
import localAttrToGlobalAttr, { ObjectAttrsWithSize, GlobalAttrs } from "../util/localAttrToGlobalAttr.js";
import type App from "./App";
import type { default as Node, LineTypes } from "./Node";
...
type DrawModes = "normal" | "ghost" | "highlight";
interface DrawNodeOptions {
  node: Node;
  mode: DrawModes;
  canvasAttr: GlobalAttrs;
}
...
interface DrawLineOptions {
  mode: DrawModes;
  lineType: LineTypes;
  nodeFromAttr: ObjectAttrsWithSize;
  nodeToAttr: ObjectAttrsWithSize;
  canvasAttr: GlobalAttrs;
}
...
export default class AppCanvas {
  ...
  private readonly canvas: LayerCanvas<typeof AppCanvas["layerNames"]>;
  ...
  private static readonly layerNames = ["info", "node", "line", "bg"] as const;

  getCanvasAttr(): GlobalAttrs {
    return {
  ...
  drawNode(options: DrawNodeOptions) {
  ...
  drawLines(options: DrawLinesOptions) {
  ...
  render(highlightNodes: Node[] = []) {
    ...
    const canvasAttr: GlobalAttrs = this.getCanvasAttr();
    ...
    for (let i = 0; i < nodes.length; i++) {
      ...
      const mode: DrawModes = isGhostNode ? "ghost" : isHightlightNode ? "highlight" : "normal";
