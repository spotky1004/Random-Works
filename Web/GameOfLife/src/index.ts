...
game.applyGame(
  "(49, 49) (49, 50) (49, 51) (50, 49) (50, 50) (50, 51) (51, 49) (51, 50) (51, 51)",
  "B1357/S02468",
  '{"backgroundColor": "#000", "lifeColor": [20, 227, 109], "deadColorStages": 4, "deadColorFrom": [20, 227, 109], "deadColorTo": [32, 36, 34]}'
);
setInterval(() => game.tick(), 50);

const elements = {
  inputs: {
    cells: document.querySelector("input[name='cells']") as HTMLInputElement,
    rules: document.querySelector("input[name='rules']") as HTMLInputElement,
    styles: document.querySelector("input[name='styles']") as HTMLInputElement,
  },
  buttons: {
    apply: document.getElementById("apply-button") as HTMLButtonElement,
    randomize: document.getElementById("randomize-button") as HTMLButtonElement
  }
};

function applyGame() {
  game.applyGame(
    elements.inputs.cells.value,
    elements.inputs.rules.value,
    elements.inputs.styles.value
  );
}
...
elements.buttons.randomize.addEventListener("click", () => {
  ...
  for (let dy = 0; dy < cells.length; dy++) {
    ...
    for (let dx = 0; dx < row.length; dx++) {
      if (row[dx] === 1) cellText += `(${x+dx}, ${y+dy}) `;
    }
  }
